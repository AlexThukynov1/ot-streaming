<template>
  <div class="home">
    ...
    <!-- Loadinf placeholder -->
    <Loader-placeholder v-if="$fetchState.pending"/>
    <!-- Movies -->
      ...
      <div v-if="searchInput === ''" id="movie-grid" class="movies-grid">
        <div  v-for="(movie, index) in movies" :key="index" class="movie">
          ...
          <div class="info">
            ...
            <NuxtLink class="button button-light"  :to="{ name: 'movies-movieid', params: { id: movie.id } }">Get more info</NuxtLink>
          </div>
        </div>
      </div>
      <div v-else id="movie-grid" class="movies-grid">
        <div  v-for="(movie, index) in searchMovieResult" :key="index" class="movie">
          <div class="movie-img">
            <img  :src="`https://image.tmdb.org/t/p/w500/${movie.poster_path}`" alt="">
            <p class="review">{{ movie.vote_average }}</p>
            <p class="overview">{{ movie.overview }}</p>
...
export default {
 ...
 methods: {
  ...
  async searchMovies () {
    ...
    const res = await data;
    res.data.results.forEach((movie) => {
      this.searchMovieResult.push(movie)
    });
  },
  clearSearch () {
    this.searchInput = '';
